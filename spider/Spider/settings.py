# 配置文件

import os
import configparser
import random

BOT_NAME = 'Spider'

SPIDER_MODULES = ['Spider.spiders']
NEWSPIDER_MODULE = 'Spider.spiders'

# 禁用 robots.txt 规则
ROBOTSTXT_OBEY = False

# 为同一网站的请求配置延迟 (默认: 0)
DOWNLOAD_DELAY = 3

# 同时处理的请求数量
CONCURRENT_REQUESTS = 8

# 每个域名同时处理的请求数量
CONCURRENT_REQUESTS_PER_DOMAIN = 4

# 每个IP地址同时处理的请求数量（Scrapy 2.9+ 需用 ScrapyPriorityQueue）
# CONCURRENT_REQUESTS_PER_IP = 1
SCHEDULER_PRIORITY_QUEUE = 'scrapy.pqueues.ScrapyPriorityQueue'

# 是否启用下载器中间件并发处理请求
CONCURRENT_REQUESTS_PER_SPIDER = 8

USER_AGENT_LIST = [
   'MSIE (MSIE 6.0; X11; Linux; i686) Opera 7.23',
   'Opera/9.20 (Macintosh; Intel Mac OS X; U; en)',
   'Opera/9.0 (Macintosh; PPC Mac OS X; U; en)',
   'iTunes/9.0.3 (Macintosh; U; Intel Mac OS X 10_6_2; en-ca)',
   'Mozilla/4.76 [en_jp] (X11; U; SunOS 5.8 sun4u)',
   'iTunes/4.2 (Macintosh; U; PPC Mac OS X 10.2)',
   'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.6; rv:5.0) Gecko/20100101 Firefox/5.0',
   'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.6; rv:9.0) Gecko/20100101 Firefox/9.0',
   'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:16.0) Gecko/20120813 Firefox/16.0',
   'Mozilla/4.77 [en] (X11; I; IRIX;64 6.5 IP30)',
   'Mozilla/4.8 [en] (X11; U; SunOS; 5.7 sun4u)'
]


# #DOWNLOADER_MIDDLEWARES = {'scrapy.downloadermiddlewares.httpproxy.HttpProxyMiddleware': 543,'Spider.middlewares.RandomProxyMiddleware': 400,}


# 重写默认请求头
DEFAULT_REQUEST_HEADERS = {
   # 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36'
   'user-agent': random.choice(USER_AGENT_LIST)
}

# 配置 item_pipelines
ITEM_PIPELINES = {
   'Spider.pipelines.SpiderPipeline': 300,
}

LOG_ENABLED = True
# 设置日志 log_file.log
# LOG_FILE = './log_file.log'
COOKIES_ENABLED = False

config = configparser.ConfigParser()

_project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_config_path = os.path.join(_project_root, 'config.ini')
_config_read = config.read(_config_path, encoding='utf-8-sig')

if _config_read:
    if config.has_section('sql'):
        TYPE = config.get('sql', 'type')
        HOST = config.get('sql', 'host')
        PORT = config.getint('sql', 'port')
        DATABASE = config.get('sql', 'db')
        USER = config.get('sql', 'user')
        PASSWORD = config.get('sql', 'passwd')
    elif config.has_section('db'):
        TYPE = config.get('db', 'type')
        HOST = config.get('db', 'host')
        PORT = config.getint('db', 'port')
        DATABASE = config.get('db', 'database')
        USER = config.get('db', 'user')
        PASSWORD = config.get('db', 'password')
    else:
        raise RuntimeError("config.ini is missing both [sql] and [db] sections")
else:
    raise FileNotFoundError(
        f"config.ini not found at {_config_path}. "
        "Copy config.ini.example to config.ini and fill in your database credentials."
    )
