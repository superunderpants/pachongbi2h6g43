	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'→'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/autohome?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.neirong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">全文</div>
						<div class="text "  >{{detail.alltext}}</div>
					</div>
					<div class="item">
						<div class="lable">浏览量(万)</div>
						<div class="text "  >{{detail.liulan}}</div>
					</div>
					<div class="item">
						<div class="lable">发布地点</div>
						<div class="text "  >{{detail.fbaddress}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('autohome','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('autohome','删除')" @click="delClick">删除</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('autohome','章节管理')">章节管理</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('autohome','查看评论')">查看评论</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('autohome/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'autohome', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			async thumbsupOrCrazily(type) {
				...
				await this.$http.post('storeup/add', this.storeupParams).then(res => {
					if (res.data.code == 0) {
						...
						this.$http.post('autohome/update', detail).then(res => {
							this.detail = detail
						});
						this.getThumbsupOrCrazilyStatus();
						this.$message({
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'autohome', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'autohome', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/autohome', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page,type=1) {
				this.$http.get('discussautohome/list', {
					params: {
						page, 
						limit: this.pageSize, 
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discussautohome/update',row).then(rs=>{
			...
			discussDel(id){
				this.$confirm('是否删除此评论？').then(_ => {
					this.$http.post('discussautohome/delete',[id]).then(res=>{
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$http.get('discussautohome/page',{params: {
							page: 1,
							limit: 1,
							refid: this.detail.id,
							userid: Number(localStorage.getItem('frontUserid'))
			...
			addComments(content=null,type=1) {
				...
				this.$http.post('discussautohome/add', data).then(rs2 => {
					if (rs2.data.code == 0) {
						if(type==1) {
							this.form.content = '';
							this.form.score = 0
			...
			addDiscussNum(type){
				...
				this.$http.get('discussautohome/list',{params: {
					page: 1,
					limit: 10000,
					refid: this.detail.id,
				}}).then(rs=>{
			...
			// 修改
			editClick(){
				this.$router.push(`/index/autohomeAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此新能源资讯？') .then(_ => {
					this.$http.post('autohome/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterautohome', query: params});
			},
			discussClick (){
				let params = {
					refid: this.detail.id
				...
				this.$router.push({path: '/index/discussautohome', query: params});
			},
			// 获取uuid
			getUUID() {
