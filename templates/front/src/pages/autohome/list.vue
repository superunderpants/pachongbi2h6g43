<template>
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('autohome','新增')" type="primary" @click="add('/index/autohomeAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('autohome','新能源资讯浏览量')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						新能源资讯浏览量
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('autohome','新能源资讯作者发布次数')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						新能源资讯作者发布次数
					</el-button>
		...
		<el-dialog
			...
			width="800">
			<div id="liulanChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.title != ''&&this.formSearch.title != undefined) searchWhere.title = '%' + this.formSearch.title + '%';
				if (this.formSearch.zuozhe != ''&&this.formSearch.zuozhe != undefined) searchWhere.zuozhe = '%' + this.formSearch.zuozhe + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`autohome/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/autohomeDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("autohome/value/title/liulan",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("autohome/group/zuozhe",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
