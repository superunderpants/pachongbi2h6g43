<template>
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-select v-model="formSearch.zixunfenlei" placeholder="请选择资讯分类" :clearable="true">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('xingyezixun','新增')" type="primary" @click="add('/index/xingyezixunAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('xingyezixun','行业资讯浏览量')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont " ></span>
						行业资讯浏览量
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('xingyezixun','行业资讯收藏量')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont " ></span>
						行业资讯收藏量
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('xingyezixun','资讯分类')" type="success" @click="chartDialogShow3">
						<span class="icon iconfont " ></span>
						资讯分类
					</el-button>
					<el-button class="list-static-btn" v-if="btnAuth('xingyezixun','日行业资讯发布量')" type="success" @click="chartDialogShow4">
						<span class="icon iconfont " ></span>
						日行业资讯发布量
					</el-button>
		...
		<el-dialog
			...
			width="800">
			<div id="zixunfenleiChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			<div v-if="changeStatQuery(['users'])" style="display: flex;width: 100%;align-items: center;margin: 0 0 20px;">
				<div style="padding: 0 10px;">发布人</div>
				<el-select v-model="chartQuery4.faburen" placeholder="请选择" @change="chartDialog4" clearable>
...
<script>
	...
	export default {
		...
		async created() {
			...
			await this.$http.get('option/zixunfenlei/zixunfenlei').then(res => {
				if (res.data.code == 0) {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.zixunbiaoti != ''&&this.formSearch.zixunbiaoti != undefined) searchWhere.zixunbiaoti = '%' + this.formSearch.zixunbiaoti + '%';
				if (this.formSearch.zixunfenlei != ''&&this.formSearch.zixunfenlei != undefined) searchWhere.zixunfenlei = this.formSearch.zixunfenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`xingyezixun/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/xingyezixunDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("xingyezixun/value/zixunbiaoti/clicknum",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("xingyezixun/value/zixunbiaoti/storeupnum",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chartDialog3() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("xingyezixun/group/zixunfenlei",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chartDialog4() {
				this.$nextTick(()=>{
					...
					this.$http.get("xingyezixun/group/fabushijian",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
