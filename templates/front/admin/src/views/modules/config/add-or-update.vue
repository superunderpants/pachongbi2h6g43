		...
		>
			<template >
				...
				<el-form-item class="upload" v-else-if="ruleForm.value" label="值" prop="value" >
					<img v-if="ruleForm.value.substring(0,4)=='http'&&ruleForm.value.split(',w').length>1" class="upload-img" style="margin-right:20px;" :key="index" :src="ruleForm.value" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.value.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" :key="index" :src="ruleForm.value.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" :key="index" v-for="(item,index) in ruleForm.value.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
			</template>
