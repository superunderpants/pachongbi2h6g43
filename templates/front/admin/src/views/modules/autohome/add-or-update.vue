		...
		>
			<template >
				...
				<el-form-item class="upload" v-else-if="ruleForm.imgurl" label="图片" prop="imgurl" >
					<img v-if="ruleForm.imgurl.substring(0,4)=='http'&&ruleForm.imgurl.split(',w').length>1" class="upload-img" style="margin-right:20px;" :key="index" :src="ruleForm.imgurl" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.imgurl.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" :key="index" :src="ruleForm.imgurl.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" :key="index" v-for="(item,index) in ruleForm.imgurl.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="浏览量(万)" prop="liulan" >
					<el-input v-model.number="ruleForm.liulan" placeholder="浏览量(万)" clearable  :readonly="ro.liulan"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="浏览量(万)" prop="liulan" >
					<el-input v-model="ruleForm.liulan" placeholder="浏览量(万)" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="发布地点" prop="fbaddress" >
					<el-input v-model="ruleForm.fbaddress" placeholder="发布地点" clearable  :readonly="ro.fbaddress"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="发布地点" prop="fbaddress" >
					<el-input v-model="ruleForm.fbaddress" placeholder="发布地点" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="简介" prop="neirong" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.neirong" label="简介" prop="neirong"  class="textBox">
			...
			<el-form-item v-else-if="ruleForm.xqurl" label="详情链接" prop="xqurl"  class="textBox">
				<span class="text">{{ruleForm.xqurl}}</span>
			</el-form-item>
			<el-form-item class="textarea" v-if="type!='info'" label="全文" prop="alltext" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
			...
			<el-form-item v-else-if="ruleForm.alltext" label="全文" prop="alltext"  class="textBox">
				<span class="text">{{ruleForm.alltext}}</span>
			</el-form-item>
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `autohome/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.autohomeCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
