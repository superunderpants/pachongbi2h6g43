		...
		>
			<template >
				...
				<el-form-item class="upload" v-else-if="ruleForm.fengmian" label="封面" prop="fengmian" >
					<img v-if="ruleForm.fengmian.substring(0,4)=='http'&&ruleForm.fengmian.split(',w').length>1" class="upload-img" style="margin-right:20px;" :key="index" :src="ruleForm.fengmian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.fengmian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" :key="index" :src="ruleForm.fengmian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" :key="index" v-for="(item,index) in ruleForm.fengmian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="资讯分类" prop="zixunfenlei" >
					<el-input v-model="ruleForm.zixunfenlei" placeholder="资讯分类" clearable  :readonly="ro.zixunfenlei"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="资讯分类" prop="zixunfenlei" >
				...
				<el-form-item class="date" v-if="type!='info'" label="解读时间" prop="jiedushijian" >
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.jiedushijian" 
						type="date"
						:readonly="ro.jiedushijian"
						placeholder="解读时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.jiedushijian" label="解读时间" prop="jiedushijian" >
					<el-input v-model="ruleForm.jiedushijian" placeholder="解读时间" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="解读内容" prop="jieduneirong" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.jieduneirong" 
					class="editor"
					myQuillEditor="jieduneirong"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.jieduneirong" label="解读内容" prop="jieduneirong"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.jieduneirong"></span>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.jiedushijian = this.getCurDate()
		},
		methods: {
			imgPreView(url){
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='jiedushijian'){
							this.ruleForm.jiedushijian = obj[o];
							this.ro.jiedushijian = true;
							continue;
						}
						if(o=='zhanghao'){
						...
						if(o=='jieduneirong'){
							this.ruleForm.jieduneirong = obj[o];
							this.ro.jieduneirong = true;
							continue;
						}
					}
				}
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jieduneirong = this.ruleForm.jieduneirong.replace(reg, this.$base.url + 'upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zixunjiedu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zixunjieduCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
