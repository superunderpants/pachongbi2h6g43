...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"bold","height":"40px"}' class="item-label">资讯标题</label>
						<el-input v-model="searchForm.zixunbiaoti" placeholder="资讯标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select" label="资讯分类" prop="zixunfenlei">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"bold","height":"40px"}' class="item-label">资讯分类</label>
						<el-select clearable v-model="searchForm.zixunfenlei" placeholder="请选择资讯分类" >
							<el-option v-for="(item,index) in zixunfenleiOptions" :key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"padding":"0 30px 30px 30px","margin":"0px 0 20px 0","borderRadius":"0 0 8px 8px ","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%"}'>
					<el-button class="add" v-if="isAuth('xingyezixun','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"16px","color":"#10C17C","height":"auto"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('xingyezixun','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('xingyezixun','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('xingyezixun','解读')" @click="zixunjieduCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								解读
							</el-button>
							<el-button class="edit" v-if=" isAuth('xingyezixun','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('xingyezixun','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('xingyezixun','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<template v-if="statType">
			<div class="statShow4">
				<div class="echarts1" v-if="isAuth('xingyezixun','行业资讯浏览量')">
					<div id="clicknumChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('xingyezixun','行业资讯收藏量')">
					<div id="storeupnumChart2" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts3" v-if="isAuth('xingyezixun','资讯分类')">
					<div id="zixunfenleiChart3" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts4" v-if="isAuth('xingyezixun','日行业资讯发布量')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery4.faburen" placeholder="请选择" @change="chartDialog4" clearable>
						<el-option v-for="item in faburenChartOptions4" :label="item" :value="item"></el-option>
					</el-select>
					<div id="fabushijianChart4" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
			</div>
		</template>
...
<script>
	...
	export default {
		...
		methods: {
			statChartClick() {
				if(this.isAuth('xingyezixun','行业资讯浏览量')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('xingyezixun','行业资讯收藏量')) {
					this.chartDialogShow2()
				}
				if(this.isAuth('xingyezixun','资讯分类')) {
					this.chartDialogShow3()
				}
				if(this.isAuth('xingyezixun','日行业资讯发布量')) {
			...
			zixunjieduCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','xingyezixun');
			...
			chartDialog3() {
				this.$nextTick(()=>{
					var zixunfenleiChart3 = echarts.init(document.getElementById("zixunfenleiChart3"),'macarons');
					let params = {
					}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							zixunfenleiChart3.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								zixunfenleiChart3.resize();
							};
						}
					})
				})
			...
			chartDialogShow4() {
				...
				this.$http.get('option/xingyezixun/faburen',
					...
					this.faburenChartOptions4 = rs.data.data
				})
				this.chartDialog4()
			},
			// 统计接口4
			chartDialog4() {
				this.$nextTick(()=>{
					...
					if(this.chartQuery4.faburen) {
						params.conditionColumn = 'faburen'
						params.conditionValue = this.chartQuery4.faburen
					}
					this.$http({
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussxingyezixun',query:{refid:id}});
			},
			// 删除
