	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="资讯分类" prop="zixunfenlei" >
					<el-select :disabled="ro.zixunfenlei" v-model="ruleForm.zixunfenlei" placeholder="请选择资讯分类"  filterable>
						<el-option
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="资讯内容" prop="zixunneirong" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
			...
			<el-form-item v-else-if="ruleForm.zixunneirong" label="资讯内容" prop="zixunneirong"  class="textBox">
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.zixunneirong = this.ruleForm.zixunneirong.replace(reg, this.$base.url + 'upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xingyezixun/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xingyezixunCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
