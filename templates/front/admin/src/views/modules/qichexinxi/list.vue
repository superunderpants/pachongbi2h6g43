...
<template>
	<div class="main-content" :style='{"padding":"22px 28px"}'>
		...
		<template v-if="showFlag  &&!statType ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"30px","borderRadius":"8px 8px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"bold","height":"40px"}' class="item-label">汽车品牌</label>
						<el-input v-model="searchForm.chepinpai" placeholder="汽车品牌" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 10px 0","display":"flex"}' class="select" label="车身类型" prop="cheshenleixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","lineHeight":"40px","fontSize":"16px","fontWeight":"bold","height":"40px"}' class="item-label">车身类型</label>
						<el-select clearable v-model="searchForm.cheshenleixing" placeholder="请选择车身类型" >
							<el-option v-for="(item,index) in cheshenleixingOptions" :key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
			...
			<div :style='{"width":"100%","padding":" 0px 15px 20px","boxShadow":"none","borderRadius":"24px","background":"#FFFFFF"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="tupian" width="200" label="封面">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<!-- [Ljava.lang.String;@3049cc88 -->
							<!-- [Ljava.lang.String;@459d22e9 -->
							<el-button class="btn8" v-if="isAuth('qichexinxi','解读')" @click="zixunjieduCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<zixunjiedu-cross-add-or-update v-if="zixunjieduCrossAddOrUpdateFlag" :parent="this" ref="zixunjieduCrossaddOrUpdate"></zixunjiedu-cross-add-or-update>



		<template v-if="statType">
			<div class="statShow4">
				<div class="echarts1" v-if="isAuth('qichexinxi','汽车信息浏览量')">
					<div id="clicknumChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('qichexinxi','汽车信息收藏量')">
					<div id="storeupnumChart2" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts3" v-if="isAuth('qichexinxi','车身类型')">
					<div id="cheshenleixingChart3" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts4" v-if="isAuth('qichexinxi','日汽车信息发布量')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery4.chepinpai" placeholder="请选择" @change="chartDialog4" clearable>
						<el-option v-for="item in chepinpaiChartOptions4" :label="item" :value="item"></el-option>
					</el-select>
					<div id="shangshishijianChart4" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
			</div>
		</template>
...
<script>
	...
	import zixunjieduCrossAddOrUpdate from "../zixunjiedu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				chepinpaiChartOptions4: [],
				chartQuery4: {},
				chartVisiable4: false,
				addOrUpdateFlag:false,
				zixunjieduCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			zixunjieduCrossAddOrUpdate,
		},
		methods: {
			statChartClick() {
				if(this.isAuth('qichexinxi','汽车信息浏览量')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('qichexinxi','汽车信息收藏量')) {
					this.chartDialogShow2()
				}
				if(this.isAuth('qichexinxi','车身类型')) {
					this.chartDialogShow3()
				}
				if(this.isAuth('qichexinxi','日汽车信息发布量')) {
			...
			zixunjieduCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.zixunjieduCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.zixunjieduCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.zixunjieduCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					this.$http({
						url: "qichexinxi/value/chepinpai/clicknum",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								// 统计图设置对了吗
								xAxis.push(res[i].chepinpai);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].chepinpai
								})
							}
							var option = {};
							let titleObj = this.bar.title
							titleObj.text = '汽车信息浏览量'
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					this.$http({
						url: "qichexinxi/value/chepinpai/storeupnum",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialog3() {
				this.$nextTick(()=>{
					var cheshenleixingChart3 = echarts.init(document.getElementById("cheshenleixingChart3"),'macarons');
					let params = {
					}
					this.$http({
						url: "qichexinxi/group/cheshenleixing",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								// 统计图设置对了吗
								xAxis.push(res[i].cheshenleixing);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].cheshenleixing
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							cheshenleixingChart3.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								cheshenleixingChart3.resize();
							};
						}
					})
				})
			...
			chartDialogShow4() {
				...
				this.$http.get('option/qichexinxi/chepinpai',
				).then(rs=>{
					this.chepinpaiChartOptions4 = rs.data.data
				})
				this.chartDialog4()
			},
			// 统计接口4
			chartDialog4() {
				this.$nextTick(()=>{
					var shangshishijianChart4 = echarts.init(document.getElementById("shangshishijianChart4"),'macarons');
					let params = {
					}
					if(this.chartQuery4.chepinpai) {
						params.conditionColumn = 'chepinpai'
						params.conditionValue = this.chartQuery4.chepinpai
					}
					this.$http({
						url: "qichexinxi/group/shangshishijian",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								// 统计图设置对了吗
								xAxis.push(res[i].shangshishijian);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].shangshishijian
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '日汽车信息发布量'
							...
							// 使用刚指定的配置项和数据显示图表。
							shangshishijianChart4.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shangshishijianChart4.resize();
							};
						}
					})
				})
			...
			init () {
				this.$http({
					url: `option/cheshenleixing/cheshenleixing`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.cheshenleixingOptions = data.data;
					} else {
			...
			getDataList() {
				...
				if(this.searchForm.chepinpai!='' && this.searchForm.chepinpai!=undefined){
					params['chepinpai'] = '%' + this.searchForm.chepinpai + '%'
				}
				if(this.searchForm.cheshenleixing!='' && this.searchForm.cheshenleixing!=undefined){
					params['cheshenleixing'] = this.searchForm.cheshenleixing
				}
