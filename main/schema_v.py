...
def schemaName_file_upload(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if file:
            ...
            file_name = "{}_{}.{}".format(int(float(time.time()) * 1000), uuid.uuid4().hex[:8], filesuffix)
...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        cmd = "cd C:/person/spader/code/code/spider && scrapy crawl "+tableName+"Spider -a databaseName=pachongbi2h6g43"
        os.system(cmd)
...
def schemaName_deepseek_askai(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "data": ""}
        req_dict = request.session.get("req_dict")
        fileName = req_dict.get('fileNames')
        content = req_dict.get('ask', '')
        content = Common.append_images_to_content_preserve_color(Common,fileName, content)
        import httpx
        custom_client = httpx.Client(trust_env=False)

        apiKey = None
        deepseek_config = config.objects.filter(name="deepseek").first()
        if deepseek_config:
            apiKey = json.loads(deepseek_config.value).get('key')
        if not apiKey:
            msg['code'] = 400
            msg['msg'] = 'DeepSeek API Key未配置'
            return JsonResponse(msg)
        client = OpenAI(api_key=apiKey, base_url="https://api.deepseek.com",http_client=custom_client)
...
def schemaName_deepseek_askai_stream(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        fileName = req_dict.get('fileNames')
        content = req_dict.get('ask', '')
        ...
        return StreamingHttpResponse(generate(), content_type='text/event-stream; charset=utf-8')
