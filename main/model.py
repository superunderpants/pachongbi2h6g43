...
class BaseModel(models.Model):
    ...
    def __Page(self, model, params, request, q):
        '''
        刷表专用
        http://ip:port/Pachongbi2h6g43/${tableName}/page
        page 当前页
        pagesize 每页记录的长度
        ...
        extra_str = ''
        if sort == 'jiage':
            extra_str = ".extra(select={'jiage_min': \"CAST(SUBSTRING_INDEX(jiage, '-', 1) AS DECIMAL(10,2))\"})"
            sort = 'jiage_min'
        ...
        datas = eval(
            '''model.objects.filter(**condition).filter(q){}{}{}{}.all()'''.format(contain_str, between_str, extra_str, order_by_str))
