...
def chat_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        req_dict = request.session.get("req_dict")
        hasUserId = 'userid' in req_dict.keys()
        global chat
        #当前登录用户信息
        ...
        if "userid" in req_dict and hasUserId:
...
def chat_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if "isreply" in  chat.getallcolumn(chat, chat):
            req_dict["isreply"]=1
        if Auth().getTokenInfo(request).get('tablename')=="users" or __isAdmin__=='是':
            req_dict["adminid"]=Auth().getTokenInfo(request).get('params').get("id")
        if Auth().getTokenInfo(request).get('tablename')!="users" and __isAdmin__!='是':
            req_dict["userid"]=Auth().getTokenInfo(request).get('params').get("id")
        #获取全部列名
        columns=  chat.getallcolumn( chat, chat)
...
def chat_add(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if "isreply" in  columns:
            req_dict["isreply"]=1
        if Auth().getTokenInfo(request).get('tablename')=="users" and Auth().getTokenInfo(request).get('params')!=None:
            req_dict["adminid"]=Auth().getTokenInfo(request).get('params').get("id")
        if Auth().getTokenInfo(request).get('tablename')!="users" and Auth().getTokenInfo(request).get('params')!=None:
            req_dict["userid"]=Auth().getTokenInfo(request).get('params').get("id")
        try:
